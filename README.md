...
*   **章節分段下拉選單（`split_by_chapter`）**: 提供三種模式，預設為「根據章編號分章輸出(第一章)」。
    *   **無**：不分章，整篇依 `max_chars_per_segment` 切段，輸出檔名也不附加 `-第X章`。
    *   **根據章編號分章輸出(第一章)**：偵測文中「第X章」標題切塊，再於章內依字數細分。
        *   章節偵測支援中文數字與阿拉伯數字（如 `第一章`、`第十二章`、`第123章`），允許前後出現括弧（`【】`、`（）`、`「」`、`《》` 等）。
        *   首段包含前言與第一章內容，之後每段對應一章，最末章延伸至檔尾。
    *   **根據字數自編章節輸出(4000字合成一章)**：適用於沒有明確章節結構的長文。
...
| `autoOutputMode` | 強制自動輸出模式：`無強制自動輸出` / `強制自動依章節分段輸出` / `強制自動依檔案數量分段輸出` |
| `seed` | 強制自動輸出時的批次索引（搭配生成後控制 fixed/increment/decrement/randomize） |
| `autoSegmentCount` | 「依檔案數量分段」每批的檔案數，預設 4 |
| `全選`、`全不選`、`反選` | 這些是按鈕，並非輸入欄位。可利用它們快速反轉目前清單的勾選狀態。|

**強制自動輸出模式（`autoOutputMode`）說明：**
*   **無強制自動輸出**：使用 `fileConfigs` 中的手動勾選狀態（原有功能）。
*   **強制自動依章節分段輸出**：由 `seed` 決定章節索引，自動選取檔名中含對應「第X章 / 第1章」的所有檔案。例：`seed=0` → 第一章所有音檔合併；`seed=1` → 第二章；依此類推。輸出檔名前綴自動附加 `-第X章`。
*   **強制自動依檔案數量分段輸出**：依 `autoSegmentCount` 將檔案列表分批，`seed` 決定批次。例：`autoSegmentCount=4`，`seed=0` 輸出第 1~4 個檔案、`seed=1` 輸出第 5~8 個、`seed=2` 輸出第 9~12 個。
...
*   **2026-06-26 (V1.10.0)**:
    *   `🌸Flower Multiline Prompt Selector` 
    ...
    - 新增 "排除純英文檔名" 勾選框(預設不勾)
    - 新增 "全部OFF"、"全部Random"、"全部Ordered" 三個按鈕。
*   **2026-06-23 (v1.8.7)**:
...
*   **2026-06-10 (v1.8.4)**:
    *   `🌸Flower Load Text From Folder` 章節分段功能大幅強化：
        ...
        *   新增 `chars_per_auto_chapter`（預設 4000）：「字數自編章節」模式的字數上限。
        *   字數自編章節模式會**於換行符號處切斷**（接近但不超過上限），避免從句子中間硬砍；尾段不足上限一半時自動併入前一塊（最後一章實際長度可達約 1.5 倍上限）。每塊自動命名 `第一章、第二章…` 並附加到輸出檔名。
    *   `🌸Flower Load Text From Folder` **Refresh 預覽強化**：按下 🔄 Refresh 會呼叫新的 `/flower-tools/preview-text-segments` 端點完整模擬節點執行，同時更新 `content_preview` 與 `file_list_display`，無需執行工作流程即可預覽分段／章節結果。
    *   `🌸Flower Audio Merge` 新增**強制自動輸出模式**：
        *   新增下拉選單 `autoOutputMode`、整數 `seed`（含 ComfyUI 生成後控制 fixed/increment/decrement/randomize）、整數 `autoSegmentCount`（預設 4）。
        *   **強制自動依章節分段輸出**：依 `seed` 自動選取檔名中含「第X章 / 第1章」的所有音檔合併，輸出檔名前綴附加 `-第X章`。
        *   **強制自動依檔案數量分段輸出**：依 `autoSegmentCount` 分批，`seed=0` → 第 1~4 個、`seed=1` → 第 5~8 個，依此類推。
*   **2026-06-07 (v1.8.3)**:
    *   `🌸Flower Load Text From Folder` 分段編號改成四位數。
*   **2026-06-07 (v1.8.2)**:
    *   `🌸Flower Load Text From Folder` 新增**「根據章節來分段」（`split_by_chapter`）** 勾選框，預設啟用：
        ...
        *   **輸出檔名自動附加章節標籤**：例如 `亦舒《喜寶》-V400-AI語音用-0001-984-第一章`，方便後續存檔時直接辨識所屬章節。
*   **2026-06-02 (v1.8.1)**:
